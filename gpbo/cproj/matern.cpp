...
double mat32(double *x1, double *x2, int d1, int d2, int D, double* ih, double* smodel){
	double r2 = 0.;
	for (int i=0; i<D; i++){
		r2+=pow((x1[i]-x2[i]),2)*ih[i+1];
	}
        double sq3r2 = SQRT3*sqrt(r2);
        double oversq3r2 = 1./sq3r2;
        double core = exp(-sq3r2);
	
	if (d1==0 and d2==0){
		//no derivatives
		return ih[0]*(1.+sq3r2)*core;
	}
	...
	if (S==1){
		...
		while (V[i]==0){i+=1;} //i no indexes he required dimension
                double x = (x1[i]-x2[i]);
		double l = ih[i+1];
                return ih[0]*l*x*(-3)*core*double(sign);
	}
	else if (S==2){
		if (P==3){
			//second derivative
			...
			double x = (x1[i]-x2[i]);
			double l = ih[i+1];
                        return ih[0]*3*l*(sq3r2-1)*core*double(sign);
		}
		else if (P==4){
...
double mat52(double *x1, double *x2, int d1, int d2, int D, double* ih, double* smodel){
	...
	//ih[i+1] = 1/l^2
        double sq5r2 = SQRT5*sqrt(r2); // r2 = d^2/l^2
